...
public class UGCaddFav {
	...
	public static class ugcMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
			...
			if (value.toString().matches(".*ugc_addFav.*")) {
				...
				String result = ip + TAB + fav_id + TAB + fav_content_id;
	...
	public static void main(String[] args) throws Exception {
		...
		String out = "/user/work/yujie/tmp/ADDFAV";
		Path path = new Path(out);
		hdfs.delete(path, true);
		UGCaddFav.runLoadMapReducue(conf, args[0], new Path(out));
	}
}
